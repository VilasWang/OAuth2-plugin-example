...
#### ~~B2. 社交账号 link/unlink~~ ✅ 已合并（PR #68，2026-08-22，v1.3.0）
...
> **交付内容**（PR #68 已合并, v1.3.0）: `GET/POST/DELETE /api/me/social/links[/{provider}]`（plan 里未列的 GET list 为自助门户必需闭环）+ identity 层 `SocialLinkService`（编排：验 code→冲突三态→插入，UNIQUE 竞态兜底）+ `ISocialAccountRepository` 扩展 4 方法（listForUser 排除 `provider='local'` 种子行）+ `GitHubAuthService::fetchProfile` 抽取（link 不触发 find-or-create）+ 最后凭证守卫（`$pbkdf2-sha256$` 前缀判定）+ userinfo numeric-dispatch（#54/#56 双键下社交会话可用）+ 4 审计事件 + user 门户 Connected Accounts 卡片（`state=link` 回调分支先于登录短路）+ 23 单测 + 10 HTTP 集成 + 5 e2e。
...
✅ B2 (社交 link/unlink)    ←── 已合并（PR #68，v1.3.0）
