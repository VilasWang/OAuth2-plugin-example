...
| **Client write path** | **None exists today** (client registration is a migration/seed-time concern, not a runtime repository operation — `IClientRepository` has no `saveClient`). When a runtime client write path is added in the future, it MUST invalidate `authforge:cache:client:{clientId}` in its success callback. |
