...
- **One switcher drives everything**: `setLocale()` updates the composer locale, pushes `services/locale.ts` state (so `normalizeError`/`getErrorMessage` follow), persists, and syncs `document.documentElement.lang`.
