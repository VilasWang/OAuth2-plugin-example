...
| P2 Lua 合并 | **已实施（2026-08-21）** | **胜出**：S6 steady 37,434→41,145（+9.9%，c256 p99 241→14ms）、S3 20,066→23,252（+15.9%），同日 A/B；5 条既有 token 缓存测试（35 断言）全过覆盖三种 EVAL 回复形态；full-test 8/8 + 静态 7/7 | 见下 |
| P3 TZ | **已实验关闭（2026-08-21）** | **无效**：TZ=UTC 同日 A/B 全档一致 -3~-11%（c128 42,110→38,384），无 ≥+3% 改善 → glibc tz 锁假设否定（~4% 栈样本判定为良性短临界区），杠杆关闭；实验编辑已还原 | 3b197fe 树 |
| P4 MGET | **已实施（2026-08-21）** | **小赢保留**：getRoles 的 MGET 顺带捞 profile 键 + 一次性进程内 memo（失效钩子同步清除，TTL 2s 兜底）；两轮同日 A/B 低并发档稳定 +9~+29%（共 5 个干净过门档），高档位噪声带内互有涨跌（本时段 p99 噪声地板 379-491ms）——收益集中于延迟敏感的低并发客户端；新 piggyback 测试 13 断言 + 3 回归全过；full-test 8/8 + 静态 7/7 | 见提交 |
