...
注：竞品列对比倍数最初为方向性推断（2026-08-18 旧表口径）—— **2026-08-21 四产品同环境全量重跑已完成**（`benchmarks/competitors/results/COMPARISON.md`，AuthForge 优化后基准档）：正式表定格 S1 2.2x / S2 2.4x / S3 1.8x / S5 1.06x（反超）/ S6 1.4x（反超），五场景全部领先；调研报告 §3.1 已同步引用。本表保留作为单一出处的终测快照。S5 的逆转（1,998→4,593）主要来自测量口径修复，部分来自写路径相关优化；旧 "S5 输 Keycloak" 结论作废。
