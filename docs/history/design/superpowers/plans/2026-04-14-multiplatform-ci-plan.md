      ...
      - name: Initialize database
        ...
        run: |
          if [ "${{ matrix.platform }}" = "windows" ]; then
            psql -h localhost -U fulla_user -d fulla_db -f sql/001_oauth2_core.sql
            psql -h localhost -U fulla_user -d fulla_db -f sql/002_users_table.sql
            psql -h localhost -U fulla_user -d fulla_db -f sql/003_rbac_schema.sql
          else
            psql -h localhost -U fulla_user -d fulla_db -f sql/001_oauth2_core.sql
            psql -h localhost -U fulla_user -d fulla_db -f sql/002_users_table.sql
            psql -h localhost -U fulla_user -d fulla_db -f sql/003_rbac_schema.sql
...
```yaml
      - name: Run tests
        ...
        env:
          FULLA_DB_HOST: "127.0.0.1"
          FULLA_REDIS_HOST: "127.0.0.1"
      ...
      - name: Print test environment
        ...
        run: |
          ...
          pg_isready -h localhost -U fulla_user || echo "pg_isready not available"
...
```markdown
![Multi-Platform CI](https://github.com/YOUR_USERNAME/fulla/workflows/Multi-Platform%20CI/badge.svg)
```
