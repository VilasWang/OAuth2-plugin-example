...
DROGON_TEST(RateLimitTest)
{
    ...
    auto runFilter = [](const std::shared_ptr<RateLimiterFilter>& filter, const HttpRequestPtr& req) -> bool {
        std::promise<bool> p;
        auto f = p.get_future();
    ...
    // Real IP Extraction Test
    {
        ...
        if (!blocked)
        {
            LOG_WARN << "Rate limit NOT enforced. Redis might be down (Fail-Open active). Skipping strict check.";
        }
        else
        {
            CHECK(blocked == true);
