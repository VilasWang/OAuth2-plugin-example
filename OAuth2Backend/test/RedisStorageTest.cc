...
DROGON_TEST(RedisStorageTest)
{
    ...
    if (!client) {
        LOG_WARN << "Redis client not available. Skipping Redis integration tests.";
        return;
    }

    // 1. Setup
    auto storage = std::make_shared<RedisOAuth2Storage>();
    // RedisStorage doesn't necessarily need initFromConfig if using global drogon redis client, 
    // but looking at valid implementation, it takes config only for creation.
    // The constructor uses "default" client name.
    ...
    // Get
    {
        ...
        auto f = p.get_future();
        storage->getAuthCode("test_redis_code_123", [&](auto c) {
            p.set_value(c);
        });
        auto c = f.get();
        if(!c.has_value()) {
