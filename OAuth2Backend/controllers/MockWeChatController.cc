...
void MockWeChatController::authorize(const HttpRequestPtr &req,
                                     std::function<void(const HttpResponsePtr &)> &&callback)
{
    ...
    std::string html = "<html><head><title>WeChat Login</title>"
                       ...
                       "<div style='width:200px; height:200px; background:#eee; margin:20px auto; line-height:200px;'>[Fake QR Code]</div>"
                       "<button onclick=\"location.href='" + redirectUri + "?code=MOCK_WECHAT_CODE&state=" + state + "'\" "
                       "style='background:#07c160; color:white; padding:10px 20px; border:none; border-radius:5px; cursor:pointer;'>Scan Success</button>"
                       "</div>"
                       "</body></html>";
...
void MockWeChatController::token(const HttpRequestPtr &req,
                                 std::function<void(const HttpResponsePtr &)> &&callback)
{
...
void MockWeChatController::userInfo(const HttpRequestPtr &req,
                                    std::function<void(const HttpResponsePtr &)> &&callback)
{
    // Handle OPTIONS for CORS
    if (req->method() == Options) {
    ...
    json["headimgurl"] = "https://ui-avatars.com/api/?name=WeChat+User&background=07c160&color=fff";
