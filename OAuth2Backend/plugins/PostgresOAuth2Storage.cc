                                      ...
                                      ClientCallback &&cb)
{
    ...
    dbClient_->execSqlAsync(
        ...
        [sharedCb, clientId](const DrogonDbException &e) {
            LOG_ERROR << "Postgres getClient DB Error for " << clientId << ": "
                      << e.base().what();
            (*sharedCb)(std::nullopt);
        },
        clientId);
                                           ...
                                           BoolCallback &&cb)
{
    ...
    // If clientSecret empty, only verify client existence (authorization
    // request)
    if (clientSecret.empty())
    {
        ...
        dbClient_->execSqlAsync(
            ...
            [sharedCb, clientId](const Result &r) {
                bool exists = !r.empty();
                LOG_DEBUG << "Postgres validateClient (no secret): clientId="
                          << clientId << " exists=" << exists;
                (*sharedCb)(exists);
            },
            [sharedCb, clientId](const DrogonDbException &e) {
                LOG_ERROR << "Postgres validateClient DB Error for " << clientId
                          << ": " << e.base().what();
                (*sharedCb)(false);
            },
            clientId);
    ...
    dbClient_->execSqlAsync(
        ...
        [sharedCb, clientSecret, clientId](const Result &r) {
            if (r.empty())
            {
                LOG_DEBUG << "Postgres validateClient: Not found -> "
                          << clientId;
                (*sharedCb)(false);
                return;
            }
                                         ...
                                         VoidCallback &&cb)
{
    ...
    dbClient_->execSqlAsync(
        "INSERT INTO oauth2_codes (code, client_id, user_id, scope, "
        "redirect_uri, expires_at, used) VALUES ($1, $2, $3, $4, $5, $6, $7)",
                                             ...
                                             VoidCallback &&cb)
{
    ...
    auto sharedCb = std::make_shared<VoidCallback>(std::move(cb));
    dbClient_->execSqlAsync(
        "UPDATE oauth2_codes SET used = true WHERE code = $1 RETURNING code",
        [sharedCb](const Result &r) {
            if (*sharedCb)
                (*sharedCb)();
        },
                                            ...
                                            VoidCallback &&cb)
{
    ...
    dbClient_->execSqlAsync(
        ...
        "expires_at, revoked) VALUES ($1, $2, $3, $4, $5, $6)",
        [sharedCb](const Result &r) {
            if (*sharedCb)
                (*sharedCb)();
        },
                                             ...
                                             VoidCallback &&cb)
{
    ...
    auto sharedCb = std::make_shared<VoidCallback>(std::move(cb));
    dbClient_->execSqlAsync(
        "INSERT INTO oauth2_refresh_tokens (token, access_token, client_id, "
        "user_id, scope, expires_at, revoked) VALUES ($1, $2, $3, $4, $5, $6, "
        "$7)",
        [sharedCb](const Result &r) {
            if (*sharedCb)
                (*sharedCb)();
