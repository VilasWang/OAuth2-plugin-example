...
void RedisOAuth2Storage::getClient(const std::string& clientId, ClientCallback&& cb) {
    ...
    redisClient_->execCommandAsync(
        [cb, clientId](const RedisResult& result) {
            ...
            if (arr.empty()) { cb(std::nullopt); return; }
            ...
            for(size_t i = 0; i < arr.size(); i += 2) {
                ...
                if (key == "secret") client.clientSecretHash = val;
                else if (key == "salt") client.salt = val;
                else if (key == "redirect_uris") {
                    auto json = parseJson(val);
                    if (json.isArray()) {
                        for (const auto& uri : json) client.redirectUris.push_back(uri.asString());
                    }
                }
            }
            cb(client);
...
void RedisOAuth2Storage::saveAuthCode(const OAuth2AuthCode& code, VoidCallback&& cb) {
    if (!redisClient_) { if(cb) cb(); return; }
    Json::Value val;
    ...
    size_t nowSec = std::chrono::duration_cast<std::chrono::seconds>(now.time_since_epoch()).count();
    ...
    redisClient_->execCommandAsync(
        ...
        "SETEX %s %s %s", key.c_str(), ttlStr.c_str(), jsonStr.c_str()
    );
}

void RedisOAuth2Storage::getAuthCode(const std::string& code, AuthCodeCallback&& cb) {
    if (!redisClient_) { cb(std::nullopt); return; }
...
void RedisOAuth2Storage::markAuthCodeUsed(const std::string& code, VoidCallback&& cb) {
    ...
    redisClient_->execCommandAsync(
        ...
        [cb](const RedisException&) { if(cb) cb(); },
        "DEL %s", key.c_str()
    );
}
...
void RedisOAuth2Storage::getAccessToken(const std::string& token, AccessTokenCallback&& cb) {
    ...
    redisClient_->execCommandAsync(
        [cb, tokenStr = token](const RedisResult& result) {
            if (result.type() == RedisResultType::kNil) { cb(std::nullopt); return; }
            std::string jsonStr = result.asString();
            auto json = parseJson(jsonStr);
            if (json.isNull()) { cb(std::nullopt); return; }
