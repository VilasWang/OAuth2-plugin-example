...
Skipped unless FULLA_BASE_URL is set. Boot recipe (Windows):
    scripts/backend/setup_database.bat     # migrations + seeds (backend-svc)
    ...start fulla-server.exe, poll /health/live...
...
BASE_URL = os.environ.get("FULLA_BASE_URL", "")
CLIENT_ID = os.environ.get("FULLA_CLIENT_ID", "backend-svc")
CLIENT_SECRET = os.environ.get("FULLA_CLIENT_SECRET", "test-secret")

pytestmark = pytest.mark.skipif(
    not BASE_URL, reason="FULLA_BASE_URL not set; integration tests need a running stack"
)
...
def test_i5_wrong_secret_raises() -> None:
    """I5 -- bad credentials surface as FullaAuthError(invalid_client)."""
