# AuthForgePackage.cmake -- shared SDK packaging helper (authforge-sdk-refactor Task 28a).
#
# Provides authforge_package(), used by every libs/* package to emit a proper
# find_package()-consumable CMake config in BOTH:
#   - the install tree (lib/cmake/<pkg>/<pkg>Config.cmake), and
#   - the build tree   (${CMAKE_BINARY_DIR}/authforge-cmake/<pkg>/<pkg>Config.cmake)
#
...
# authforge::authforge-<pkg> instead of the canonical authforge::<alias> that
...
# find_package() (CMAKE_PREFIX_PATH pointing at AUTHFORGE_BUILD_CMAKE_DIR)
...
# first package's scope, and authforge_package() is later called from other
...
set(AUTHFORGE_PACKAGE_MODULE_DIR "${CMAKE_CURRENT_LIST_DIR}"
    CACHE INTERNAL "Directory containing AuthForgePackage.cmake + its template")
...
# adds this root to CMAKE_PREFIX_PATH to resolve the entire authforge closure
# (find_package searches <prefix>/<name>/).
if(NOT DEFINED AUTHFORGE_BUILD_CMAKE_DIR)
    set(AUTHFORGE_BUILD_CMAKE_DIR "${CMAKE_BINARY_DIR}/authforge-cmake"
        CACHE INTERNAL "Root dir for authforge build-tree package configs")
endif()

# authforge_package(
#     TARGET       <real-target-name>          # required, e.g. authforge-common
#     PACKAGE      <find_package-name>          # optional, defaults to TARGET
#     EXPORT_NAME  <authforge::<name> suffix>   # required, e.g. common / storage::memory
#     DEPENDENCIES <pkg> [<pkg> ...]            # find_dependency() closure (plain names)
# )
function(authforge_package)
    set(options)
    ...
    if(NOT AP_TARGET)
        message(FATAL_ERROR "authforge_package: TARGET is required")
    endif()
    if(NOT AP_PACKAGE)
    ...
    if(NOT AP_EXPORT_NAME)
        message(FATAL_ERROR
            "authforge_package: EXPORT_NAME is required "
            "(the authforge:: alias suffix, e.g. common / storage::memory)")
    endif()

    set(_export_set "${AP_PACKAGE}Targets")
    set(_version "${OAUTH2_PROJECT_VERSION}")
    if(NOT _version)
        set(_version "1.0.0")
    endif()

    # Consumers get authforge::<export-name>, matching the in-tree ALIAS and
    # design.md's canonical names (without this, NAMESPACE authforge:: would
    # yield authforge::authforge-<pkg>).
    ...
    set(AUTHFORGE_FIND_DEPENDENCY_BLOCK "")
    foreach(_dep IN LISTS AP_DEPENDENCIES)
        string(APPEND AUTHFORGE_FIND_DEPENDENCY_BLOCK "find_dependency(${_dep})\n")
    endforeach()
    set(AUTHFORGE_PKG_NAME "${AP_PACKAGE}")

    set(_template "${AUTHFORGE_PACKAGE_MODULE_DIR}/AuthForgePackageConfig.cmake.in")

    # --- install tree: Config + ConfigVersion + EXPORT ----------------------
    set(_install_cmakedir "${CMAKE_INSTALL_LIBDIR}/cmake/${AP_PACKAGE}")
    set(_install_stage "${CMAKE_CURRENT_BINARY_DIR}/authforge-package/install")
    ...
    set(_build_cmakedir "${AUTHFORGE_BUILD_CMAKE_DIR}/${AP_PACKAGE}")
