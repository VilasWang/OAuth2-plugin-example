...
class PostgresRepositoryBase
{
  ...
  protected:
    ...
    // fulla::storage::postgres, bare "drogon::" can resolve to
    // fulla::storage::postgres::drogon if such a member exists elsewhere
    // in the include graph (OAuth2Plugin's StorageSubjectResolver.h declares
    // namespace fulla::drogon::adapters). Globally qualify to be safe.
