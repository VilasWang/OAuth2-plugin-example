...
// Task 19 (fulla-sdk-refactor, design.md §5.1/§5.4/§6): Adapter-layer
...
// fulla::identity::IdentityService (still the production path for the
...
// backing store, used only by the new fulla::identity::AuthService /
 ...
 * split it then.
 */
class PostgresIdentityRepository : public fulla::identity::IUserRepository,
                                   public fulla::identity::IRoleRepository,
                                   public fulla::identity::ISubjectMappingRepository,
                                   public std::enable_shared_from_this<PostgresIdentityRepository>
{
  public:
    // M3 Task 20 pitfall (see PROGRESS.md's "fulla::drogon::* 命名空间
    // 裸写" note): globally qualified (::drogon::orm::DbClientPtr) so this
    // header compiles correctly in any translation unit that also
    // includes an fulla::drogon::* header (e.g. bootstrap/
    // IdentityAssembly.cc, which includes
    // fulla/drogon/controllers/SessionController.h in the same TU) --
    // an unqualified `drogon::` here would resolve to the sibling
    // fulla::drogon namespace instead of the global ::drogon one.
    ...
    void findByEmail(
      ...
      std::function<void(std::optional<fulla::identity::UserData>)> &&callback
    ) override;
    ...
    void create(
      const fulla::identity::UserData &userData,
