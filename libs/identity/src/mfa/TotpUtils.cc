...
std::string base32Encode(const uint8_t *data, size_t len)
{
    ...
    // #104 (first ASan+UBSan run): the accumulation buffers in both base32
    // directions never dropped consumed bits, so `buffer` grew 5-8 bits per
    // input unit and overflowed -- signed-int left shift, i.e. UB that
    // happened to wrap benignly on two's-complement targets until
    // -fno-sanitize-recover=undefined aborted the TOTP suite. Keep the live
    // bits only (mask after each emit); emitted values read exactly those
    // bits, so outputs are byte-identical to the wrapped-int behavior.
    uint32_t buffer = 0;
    int bitsLeft = 0;
