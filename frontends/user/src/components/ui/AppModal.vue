...
const props = defineProps<{
  open: boolean
  title?: string
  /** Accessible name when the header row is suppressed (blocking dialogs
      rendered without title/header slot). Falls back to title. */
  ariaLabel?: string
  size?: 'sm' | 'md' | 'lg'
}>()
    ...
    <Transition name="modal">
      ...
      >
        ...
        <div
          ...
          tabindex="-1"
          :class="[
            'relative w-full bg-surface rounded-card shadow-xl border border-neutral-200',
        ...
        >
          ...
          <div
            v-if="$slots.footer"
            class="flex items-center justify-end gap-3 px-6 py-4 border-t border-neutral-100 bg-neutral-50 rounded-b-card"
          >
            <slot name="footer" />
          </div>
        </div>
