/**
 * Simplified-Chinese (zh-CN) error message resources — PLACEHOLDER / SEED.
 *
 * Task 9.3 owns this file and will complete + verify FULL coverage of every
 * backend Error_Code and every OAuth2 protocol code, plus add any other
 * locales. This seed contains the reserved fallback keys plus the catalog
 * entries already defined in design.md so that `errorAdapter.ts` compiles and
 * is functional in the meantime.
 *
 * Invariants task 9.3 must preserve (Requirements 9.1, 9.4):
...
export const zhCN: Record<string, string> = {
  ...
  VALIDATION_PASSWORD_TOO_SHORT: '密码长度不足',
