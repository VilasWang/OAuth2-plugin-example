 ...
 *
 * Mirrors every key of `./zh-CN.ts` (backend Error_Catalog codes, OAuth2 /
 * RFC 6749 / 7009 / 8628 / 6750 protocol codes, OIDC prompt codes, and the
 * reserved `__unknown__` / `__network__` fallbacks) — enforced by
 * Property 13 across every registered locale.
 *
 * Authoring constraints (Property 13 tripwires — see
 * docs/contribute/frontend-i18n.md): no "exception"/"traceback"/"stack
 * trace" wording, no `{{`/`}}`/`${`/`%s`/`%d` placeholder markers; the
 * property patterns must never be loosened to admit new copy.
 */
