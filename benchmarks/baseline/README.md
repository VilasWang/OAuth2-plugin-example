...
| **Measured params** | Per level: **5 s warmup + 10 s measured** (`WARMUP_S=5 DURATION_S=10`, the session protocol of `run-authforge-session.sh`) — **not** the `run-scenario.sh` defaults of 10 s + 30 s. Future A/B runs must reuse the measured params (same windows) to stay comparable. |
