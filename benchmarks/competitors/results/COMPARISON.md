...
|---|---|---|---|---|---|---|
| AuthForge | **87,499** | c=16 | 0.2ms | 0.3ms | 3.5ms | 0.0091% |
...
| 产品 | 稳态 QPS | 稳态档 c | P50 | P90 | P99 | 错误率 |
|---|---|---|---|---|---|---|
| AuthForge | **14,438** | c=64 | 4.2ms | 6.3ms | 10.9ms | 0.0000% |
...
| 产品 | 稳态 QPS | 稳态档 c | P50 | P90 | P99 | 错误率 |
|---|---|---|---|---|---|---|
| AuthForge | **22,458** | c=64 | 2.7ms | 4.1ms | 402.2ms | 0.0000% |
...
| 产品 | 稳态 QPS | 稳态档 c | P50 | P90 | P99 | 错误率 |
|---|---|---|---|---|---|---|
| AuthForge | **5,506** | c=64 | 11.2ms | 15.5ms | 23.2ms | 0.0000% |
...
| 产品 | 稳态 QPS | 稳态档 c | P50 | P90 | P99 | 错误率 |
|---|---|---|---|---|---|---|
| AuthForge | **49,302** | c=128 | 2.5ms | 3.8ms | 5.6ms | 0.0000% |
...
|---|---|---|---|---|---|---|
| AuthForge | s6-userinfo | 30 | 3.2ms | 1800.0ms | 566.93x | 9 |
| Keycloak | s6-userinfo | 30 | 4.8ms | 1830.0ms | 384.86x | 10 |
| Ory Hydra | s6-userinfo | 30 | 24.5ms | 1830.0ms | 74.65x | 10 |
| Zitadel | s6-userinfo | 30 | 18.9ms | 1850.0ms | 97.99x | 11 |

逐段 P99（ms）：

- **AuthForge**: 2, 1790, 2.3, 2, 1800, 2.2, 2.1, 1780, 2.2, 2.6, 1770, 2.6, 3, 1780, 2.9, 3.2, 1770, 3.1, 3, 273.9, 3, 3.4, 3.4, 2.8, 3.4, 1800, 3.5, 687.8, 3.5, 3
...
> **诚实注记（G4，2026-08-21 重跑修订）**：本次四家在同机同时段**全部出现同款周期尖峰**（Keycloak 最大/中位 384.86x、Ory/Zitadel 亦数十倍）——跨产品同款尖峰直接证明这是宿主环境层噪声（WSL2 调度/IO 停顿），而非任何一家的运行时行为；各产品的 GC 差异在本口径下不可分辨。可比较的是绝对水位与比值：AuthForge 中位 P99 3.2ms、最大/中位 566.93x（vs Ory Hydra 24.5ms / Zitadel 18.9ms）——中位与比值均最优或并列最优时才可引用本表。
