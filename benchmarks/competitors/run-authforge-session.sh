#!/usr/bin/env bash
# benchmarks/competitors/run-authforge-session.sh
#
# AuthForge side of the same-session comparison (design §5.1 v1.1): re-run the
...
#
# Usage:
#   bash benchmarks/competitors/run-authforge-session.sh
#
# Assumes: docker stack down (run-authforge-session calls authforge setup.sh
# itself, which tears down any previous bench stack first).
set -euo pipefail

COMP_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$COMP_DIR/../.." && pwd)"
AF_BENCH="$REPO_ROOT/benchmarks/authforge"
RUNNER="$AF_BENCH/run-scenario.sh"
RESULTS_DIR="$REPO_ROOT/benchmarks/results"     # canonical AuthForge location
...
export RESULTS_DIR
export BENCH_PRODUCT=authforge
export BENCH_PRODUCT_VERSION="$(cd "$REPO_ROOT" && git rev-parse --short HEAD)"

echo "== AuthForge benchmark stack setup =="
