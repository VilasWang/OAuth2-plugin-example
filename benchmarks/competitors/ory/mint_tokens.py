...
def oauth2_auth_flow(public: str, admin: str, client_id: str, client_secret: str,
    ...
    # 2. accept login (admin API; v2 uses PUT)
    r2 = s.put(admin + "/admin/oauth2/auth/requests/login/accept",
    ...
    # 4. accept consent (admin API; v2 uses PUT)
    r4 = s.put(admin + "/admin/oauth2/auth/requests/consent/accept",
...
def mint_one(args: argparse.Namespace, session: requests.Session) -> str:
    if args.mode == "cc-token":
        ...
        resp = oauth2_auth_flow(args.public, args.admin, args.client_id,
                                args.client_secret, args.redirect_uri, session)
...
def main() -> int:
    ...
    p.add_argument("--ca", default=str(Path(__file__).resolve().parent
                                       / "lib" / "generated" / "tls" / "cert.pem"),
                   help="CA bundle for the https public port (the bench stack's "
                        "self-signed cert, generated by setup.sh)")
    args = p.parse_args()

    # Hydra v26 production mode serves TLS with a setup-time self-signed cert;
    # verification stays ON with that cert pinned as the CA.
    if args.public.startswith("https"):
        if not Path(args.ca).is_file():
            print(f"error: --ca not found: {args.ca}", file=sys.stderr)
            return 2
        print(f"[mint] TLS verification against pinned CA {args.ca}", file=sys.stderr)
    ...
    def work(i: int) -> None:
        ...
        if args.public.startswith("https"):
            session.verify = args.ca  # pinned self-signed CA (setup.sh generates)
        tok = None
