#!/usr/bin/env bash
# benchmarks/fulla/pool-sweep.sh
#
# DB pool capacity sweep (noncode-performance-optimization.md §五.1 / §八 step 2).
# Pool 25 is the four-product ALIGNMENT value, not an Fulla optimum — the
...
#
# Usage:
#   bash benchmarks/fulla/pool-sweep.sh [pool ...]     # default: 25 64 100
#
# Protocol:
#   - pool #1 runs a FULL deterministic setup (down -v; fresh PG with current
#     migrations, seed, token pools, user warmup) — reuse benchmarks/fulla/
#     setup.sh machinery.
#   - later pools keep the stack (no volume reset), rewrite the db pool in the
#     swapped-in bench config.json and `docker restart fulla-backend`
...
snapshot_env() {
    {
        ...
        docker exec fulla-postgres psql -U fulla_user -d fulla_db -tAc \
            "SELECT version()" | head -1
        for guc in shared_buffers effective_cache_size checkpoint_timeout max_wal_size wal_compression; do
            printf '%s = ' "$guc"
            docker exec fulla-postgres psql -U fulla_user -d fulla_db -tAc "SHOW $guc"
        done
        echo "pg rows: tokens=$(docker exec fulla-postgres psql -U fulla_user -d fulla_db -tAc 'SELECT count(*) FROM oauth2_access_tokens' | tr -d ' ') audit=$(docker exec fulla-postgres psql -U fulla_user -d fulla_db -tAc 'SELECT count(*) FROM audit_logs' | tr -d ' ')"
...
for idx in "${!POOLS[@]}"; do
    ...
    set_pool "$pool"
    docker restart fulla-backend >/dev/null
    wait_ready
    snapshot_env
