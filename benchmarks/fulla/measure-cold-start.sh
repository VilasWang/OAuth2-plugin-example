...
#   Mode A (auto-migrate): FULLA_AUTO_MIGRATE=true → cold start INCLUDES
...
#
# Usage:
#   bash benchmarks/fulla/measure-cold-start.sh [--pre-migrated]
#
...
BENCH_CONFIG="$REPO_ROOT/${FULLA_SERVER_DIR:-apps/server}/config/config.bench.json"
DEV_CONFIG="$REPO_ROOT/${FULLA_SERVER_DIR:-apps/server}/config/config.json"
DEV_CONFIG_BACKUP="$REPO_ROOT/${FULLA_SERVER_DIR:-apps/server}/config/config.json.dev-backup"
...
echo "[cold-start] mode: $([ "$PRE_MIGRATED" = "1" ] && echo "pre-migrated (--migrate-only)" || echo "auto-migrate (FULLA_AUTO_MIGRATE=true)")"

# --- ensure DB is up (postgres + redis) but backend is NOT running ---
echo "[cold-start] stopping backend (if running)..."
docker compose "${COMPOSE_ARGS[@]}" --project-directory "$REPO_ROOT" stop fulla-backend >/dev/null 2>&1 || true
...
if [ "$PRE_MIGRATED" = "1" ]; then
    ...
    docker compose "${COMPOSE_ARGS[@]}" --project-directory "$REPO_ROOT" run --rm \
        fulla-backend --migrate-only >/dev/null 2>&1 || echo "[cold-start] WARN: --migrate-only failed (may need FULLA_AUTO_MIGRATE=false)"
fi
...
docker compose "${COMPOSE_ARGS[@]}" --project-directory "$REPO_ROOT" up -d fulla-backend >/dev/null 2>&1
...
RSS_BYTES=0
for i in $(seq 1 4); do
    MEM="$(docker stats --no-stream --format "{{.MemUsage}}" $(docker compose "${COMPOSE_ARGS[@]}" --project-directory "$REPO_ROOT" ps -q fulla-backend 2>/dev/null) 2>/dev/null | head -1 || true)"
