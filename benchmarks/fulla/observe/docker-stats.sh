...
#                    Default: *fulla-backend*|*fulla-postgres*|*fulla-redis*
...
# Default targets the Fulla stack; competitor runs override with
# CONTAINER_GLOB (a case-pattern, e.g. 'bench-keycloak*|bench-postgres*').
CONTAINER_GLOB="${CONTAINER_GLOB:-*fulla-backend*|*fulla-postgres*|*fulla-redis*}"
...
while true; do
    ...
    # NOTE: the glob may contain '|' alternatives (the fulla default has
