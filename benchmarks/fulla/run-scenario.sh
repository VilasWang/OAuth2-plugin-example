#!/usr/bin/env bash
# benchmarks/fulla/run-scenario.sh
#
...
#
# Usage:
#   bash benchmarks/fulla/run-scenario.sh <scenario.lua> [conn ...]
#   bash benchmarks/fulla/run-scenario.sh scenarios/s2-client-credentials.lua 2 4 8
#   bash benchmarks/fulla/run-scenario.sh scenarios/s1-discovery.lua   # default staircase
#   bash benchmarks/fulla/run-scenario.sh scenarios/s5-refresh-token.lua --reseed lib/generated/bench_refresh_tokens.sql
#
# Options:
...
#                        /metrics endpoint, so scrape-metrics is Fulla-only).
...
#   BENCH_PRODUCT          default fulla (recorded in result env block;
#                          non-fulla also adds a <product> infix to the
#                          result filename)
...
BENCH_PRODUCT="${BENCH_PRODUCT:-fulla}"
...
if [ -z "$SCENARIO_REL" ]; then
    echo "Usage: bash benchmarks/fulla/run-scenario.sh <scenario.lua> [conn ...]"
...
#                 Default is the Fulla bench lib; competitor runs override
#                 it so their scenarios resolve THEIR pools, not Fulla's.
...
for CONN in "${LEVELS[@]}"; do
    ...
    if [ -n "$RESEED_SQL_ABS" ]; then
        ...
        PG_CONTAINER="$(docker ps -q --filter 'name=fulla-postgres' --filter 'status=running' 2>/dev/null | head -1)"
        if [ -n "$PG_CONTAINER" ]; then
            ...
            { echo "TRUNCATE oauth2_refresh_tokens RESTART IDENTITY CASCADE;"; cat "$RESEED_SQL_ABS"; } \
                | docker exec -i "$PG_CONTAINER" psql -U fulla_user -d fulla_db -v ON_ERROR_STOP=1 -q >/dev/null 2>&1 \
    ...
    if [ -n "$RESEED_SQL_ABS" ]; then
        PG_CONTAINER_FRESH="$(docker ps -q --filter 'name=fulla-postgres' --filter 'status=running' 2>/dev/null | head -1)"
        if [ -n "$PG_CONTAINER_FRESH" ]; then
            { echo "TRUNCATE oauth2_refresh_tokens RESTART IDENTITY CASCADE;"; cat "$RESEED_SQL_ABS"; } \
                | docker exec -i "$PG_CONTAINER_FRESH" psql -U fulla_user -d fulla_db -v ON_ERROR_STOP=1 -q >/dev/null 2>&1 \
