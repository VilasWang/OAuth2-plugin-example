...
echo "FULLA_DB_HOST: $FULLA_DB_HOST"
echo "FULLA_REDIS_HOST: $FULLA_REDIS_HOST"
...
COUNT=0
until pg_isready -h "$FULLA_DB_HOST" -p "${FULLA_DB_PORT:-5432}" -U "$FULLA_DB_USER" > /dev/null 2>&1 || [ $COUNT -eq $MAX_RETRIES ]; do
    echo -e "${YELLOW}Waiting for PostgreSQL at $FULLA_DB_HOST ($COUNT/$MAX_RETRIES)...${NC}"
    sleep 1
    COUNT=$((COUNT + 1))
done
...
COUNT=0
until redis-cli -h "$FULLA_REDIS_HOST" -p "${FULLA_REDIS_PORT:-6379}" -a "$FULLA_REDIS_PASSWORD" ping > /dev/null 2>&1 || [ $COUNT -eq $MAX_RETRIES ]; do
    echo -e "${YELLOW}Waiting for Redis at $FULLA_REDIS_HOST ($COUNT/$MAX_RETRIES)...${NC}"
    sleep 1
    COUNT=$((COUNT + 1))
done
...
export PGPASSWORD=$FULLA_DB_PASSWORD
# Apply all migrations in order
for migration in /app/sql/migrations/V*.sql; do
    psql -h "$FULLA_DB_HOST" -p "${FULLA_DB_PORT:-5432}" -U "$FULLA_DB_USER" -d "$FULLA_DB_NAME" -f "$migration" > /dev/null 2>&1
done
# Apply seed data
for seed in /app/sql/seed/*.sql; do
    psql -h "$FULLA_DB_HOST" -p "${FULLA_DB_PORT:-5432}" -U "$FULLA_DB_USER" -d "$FULLA_DB_NAME" -f "$seed" > /dev/null 2>&1
done
