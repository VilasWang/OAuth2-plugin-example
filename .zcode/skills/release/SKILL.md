...
### 1. 版本号确认
版本号的**唯一真相来源**是 `cmake/Version.cmake`（`OAUTH2_PROJECT_VERSION_MAJOR` / `_MINOR` / `_PATCH` → `OAUTH2_PROJECT_VERSION`，当前 `1.1.0`）。`apps/server/CMakeLists.txt` 与 `libs/drogon/CMakeLists.txt` 均引用 `${OAUTH2_PROJECT_VERSION}`（`libs/drogon` 在未传播时回退默认 `1.0.0`）。确认这些文件解析出的版本一致（语义化版本：MAJOR.MINOR.PATCH）。
